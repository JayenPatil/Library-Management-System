...
-- Task 3: Delete a Record from the Issued Status Table. Objective: Delete the record with issued_id = 'IS121' from the issued_status table.alter
...
select issued_emp_id, count(*) as issued_book from issued_status
...
select category,sum(rental_price) as total_renatl ,count(*) as book_count
from books as b
...
select e.emp_name,b.branch_address as branch,count(ist.issued_id) as no_book_issued
from issued_status as ist
join employees as e on e.emp_id = ist.issued_emp_id
join branch as b on e.branch_id = b.branch_id
